%% This is mainstream file based on transformers for preparing obfuscated Node based on Laplace + Trasprotect
...
%                 loc_set_z_vec(1, k) = exp(-distance*100);     %EPSILON = 100
                loc_set_z_vec(1, loc_idx) = exp(-distance*50);     %EPSILON = 100
            end
            ...
            obf_node = obfuscation1(new_df_nodes, loc_set_z_vec);      % obfuscation of node
            obf_traj{end+1} = obf_node;
        end
        all_obf_traj{end+1, 1} = obf_traj;
    end
    pv{end+1, 1} = post_vectors;
%     disp("Batch No.")
%     disp(a)
% end
toc
