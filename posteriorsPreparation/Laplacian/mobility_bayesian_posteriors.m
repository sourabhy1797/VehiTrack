...
%     obf_idx = find(new_node_ids == obf_node);
    ...
    for k = 1 : length(new_node_ids)
        [path, distances] = shortestpath(G, new_node_ids(obf_node), new_node_ids(k));
        dist = [dist, distances];
        if distances < 250
    ...
    probable_loc = [probable_loc, new_node_ids(obf_node)];
...
%         j =2;
            ...
            for m = 1 : length(new_node_ids)
                ...
                if isfinite(distances)
                    if ~ismember(probable_loc(k), new_probable_loc)
                        [obf_path, obf_dist] = shortestpath(G, probable_loc(k), new_node_ids(obf_node));
                        if(obf_dist < 100)
        ...
        probable_loc = [new_probable_loc,  new_node_ids(obf_node)];
